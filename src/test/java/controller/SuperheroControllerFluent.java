...
public class SuperheroControllerFluent {
    ...
    public ApiHttpResponse createSuperhero(SuperheroWithBuilder superhero) {
        ...
        return new ApiHttpResponse(given(requestSpecification).when().post("superheroes").then());
    }

    // GET - get
    @Step("Get superhero by id")
    public ApiHttpResponse getSuperheroesById(Integer id) {
        return new ApiHttpResponse(given(requestSpecification).when().get("superheroes/" + id).then());
    }
    ...
    public ApiHttpResponse getSuperheroesByIdWithRetry(Integer id) throws InterruptedException {
        ...
        if (statusCode != 200) {
            for (int i = 0; i < 10; i++) {
                ...
                Thread.sleep(1000);
                response = given(requestSpecification).when().get("superheroes/" + id);
    ...
    public ApiHttpResponse getAllSuperheroes() {
        return new ApiHttpResponse(given(requestSpecification).when().get("superheroes").then());
    }
    ...
    public ApiHttpResponse updateSuperhero(SuperheroWithBuilder superhero, Integer id) {
        requestSpecification.body(superhero);
        return new ApiHttpResponse(given(requestSpecification).when().put("superheroes/" + id).then());
    }
    ...
    public ApiHttpResponse updateSuperheroWithRetry(SuperheroWithBuilder superhero, Integer id) throws InterruptedException {
        requestSpecification.body(superhero);
        Response response = given(this.requestSpecification).when().put("superheroes/" + id);
        int statusCode = response.getStatusCode();
        if (statusCode != 200) {
            for (int i = 0; i < 10; i++) {
                System.out.println("Waiting total " + (i + 1) + " second");
                Thread.sleep(1000);
                response = given(this.requestSpecification).when().put("superheroes/" + id);
                statusCode = response.getStatusCode();
                if (statusCode == 200) {
                    break;
                }
    ...
    public ApiHttpResponse deleteSuperheroById(Integer id) {
        return new ApiHttpResponse(given(requestSpecification).log().all().when().delete(String.format("superheroes/" + id)).then());
    }
}
